...
interface User { id: string; email: string; name: string; role: string; apps: string[] }
...
function Login({ onLogin }: { onLogin: (user: User) => void }) {
  ...
  const testUsers: Record<string, { password: string; user: User }> = {
    'admin@trivium.com': { password: 'admin123', user: { id: '1', email: 'admin@trivium.com', name: 'Admin Trivium', role: 'admin', apps: ['voltride-opp', 'motorrent-opp', 'backoffice-voltride', 'backoffice-motorrent', 'trivium-buggy', 'comptabilite', 'mecanique', 'stock'] } },
    'voltride@trivium.com': { password: 'voltride123', user: { id: '2', email: 'voltride@trivium.com', name: 'Opérateur Voltride', role: 'operator', apps: ['voltride-opp'] } },
    'motorrent@trivium.com': { password: 'motorrent123', user: { id: '3', email: 'motorrent@trivium.com', name: 'Opérateur Motor Rent', role: 'operator', apps: ['motorrent-opp'] } }
  }
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 w-full max-w-md border border-white/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full py-3 px-4 bg-gradient-to-r from-blue-500 to-purple-600 text-white font-semibold rounded-lg hover:from-blue-600 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 transition-all">{loading ? 'Connexion...' : 'Se connecter'}</button>
        </form>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
        {userApps.map(app => (
          <a key={app.id} href={app.comingSoon ? '#' : app.url} target={app.comingSoon ? '_self' : '_blank'} rel="noopener noreferrer" className={`group relative bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 transition-all hover:scale-105 hover:bg-white/15 ${app.comingSoon ? 'cursor-not-allowed opacity-60' : ''}`} onClick={e => app.comingSoon && e.preventDefault()}>
            ...
            <div className="w-16 h-16 rounded-xl flex items-center justify-center mb-4 text-3xl" style={{ backgroundColor: app.color + '30' }}>
              {app.icon.startsWith('http') ? <img src={app.icon} alt={app.name} className="w-10 h-10 object-contain" /> : app.icon}
            </div>
...
export default function App() {
  ...
  useEffect(() => { const savedUser = localStorage.getItem('trivium_user'); if (savedUser) { setUser(JSON.parse(savedUser)) }; setLoading(false) }, [])
  if (loading) { return <div className="min-h-screen flex items-center justify-center"><div className="text-white text-xl">Chargement...</div></div> }
  if (!user) { return <Login onLogin={(userData) => setUser(userData)} /> }
  return <Dashboard user={user} onLogout={() => { localStorage.removeItem('trivium_user'); setUser(null) }} />
}